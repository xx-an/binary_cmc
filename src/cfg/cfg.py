...
class CFG(object):
        ...
        if inst.startswith('ret') or inst.endswith(' ret'):
            ...
            if new_address in self.address_inst_map:
                if new_address in self.address_ext_func_map:
                    if address not in self.ext_func_call_instr_addr:
                        self.ext_func_call_instr_addr.append(address)
        ...
        if utils.check_not_single_branch_inst(inst):    # je xxx
            ...
            if new_address in self.address_block_map and new_address in self.ret_call_address_map.values():
                if self._explored_func_block(sym_store, new_address):
                    if new_address not in self.ext_func_call_instr_addr:
    ...
    def _locate_pointer_related_error(self, block, init_sym_store, address, inst, sym_names):
        ...
        utils.output_logger.info('Trace back to ' + str(sym_names) + ' after ' + hex(block.address) + ': ' + block.inst)
        tb_halt_point = False
        node_stack = []
        src_names = None
        ...
        while node_stack:
            ...
            for sym_name in sym_names:
                ...
                utils.logger.info(src_names)
                utils.output_logger.info('Trace back to ' + str(src_names) + ' after ' + hex(block.address) + ': ' + block.inst)
